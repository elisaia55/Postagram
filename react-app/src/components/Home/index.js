...
const Home = () => {
        ...
        useEffect(() => {
            const handleClick = (e) => {
                if (
                    ...
                    !e?.target?.nextElementSibling?.classList.contains("hidden2")
                ) {
                    setOpen(0);
                }
                ref.current.forEach((r, i) => {
                    if (ref.current[i] && !ref.current[i].contains(e.target)) {
                        emojis.current[i].classList.add("hidden2");
                    }
                });
            };
    ...
    const loadIt = (i) => {
        let arr = [...loaded];
        document.querySelector(`.post-img-${i}`).classList.add("hidden2");
        document.querySelector(`.loaded-img-${i}`).classList.remove("hidden2");
        arr[i] = true;
        setLoaded(arr);
    };
    ...
    const show = (i) => {
        if (open === 0) {
            emojis.current[i].classList.remove("hidden2");
            setOpen(1);
        } else {
            setOpen(0);
        }
                        ...
                        <div className="home-posts">
                            { followingPosts?.slice(0, 20).map((post, i) => (
                                <div key={ post + i } className="post-card">
                                    ...
                                    />
                                    <img
                                        className={ `img-absolute liked-img-${post.post.id} hidden2` }
                                        src={ src }
                                    />
